...
function initConfetti(canvasId: string): void {
...
function burstConfetti(config: ConfettiConfig): void {
  const {
    x, y,
    count   = 60,
    colors  = ['#f472b6','#fbbf24','#fb7185','#a78bfa','#34d399','#60a5fa'],
    spread  = 12,
    gravity = 0.3,
  } = config;
...
const zoneL  = document.getElementById('zone-left')  as HTMLDivElement;
const zoneR  = document.getElementById('zone-right') as HTMLDivElement;
let current     = 0;
...
function getDots(): HTMLElement[] { return Array.from(dotsEl.querySelectorAll('.dot')); }

function triggerStagger(card: HTMLElement): void {
  card.querySelectorAll<HTMLElement>('.stagger-item').forEach((el, i) => {
...
function playCoverEntrance(): void {
  const cake    = document.querySelector<HTMLElement>('#card-1 .cake-emoji');
  const title   = document.querySelector<HTMLElement>('#card-1 .cover-title');
  const name    = document.querySelector<HTMLElement>('#card-1 .cover-name');
  const sub     = document.querySelector<HTMLElement>('#card-1 .cover-sub');
  const hint    = document.querySelector<HTMLElement>('#card-1 .tap-hint');
  const ribbon  = document.querySelector<HTMLElement>('#card-1 .ribbon');
...
const galleryBody = document.getElementById('gallery-body') as HTMLDivElement | null;
...
document.querySelectorAll<HTMLElement>('.present').forEach(present => {
